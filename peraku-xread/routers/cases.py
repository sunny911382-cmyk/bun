...
def _assert_owner(case: dict, user: dict) -> None:
    if not case:
        raise HTTPException(404, "Case not found")
    if str(case.get("user_id")) != str(user.id):
...
def create_case(
    ...
    case = cases_repo.create_case(user_type.value, meta, user_id=str(user.id))
...
async def upload_document(
    ...
    existing = [d for d in existing if d.get("doc") != doc_id]
...
async def run_credit_profile(
    case_id: str,
    data: CreditProfileInput,
    user: dict = Depends(current_user),
):
    case = cases_repo.get_case(case_id)
    _assert_owner(case, user)
...
def list_cases(user: dict = Depends(current_user)):
    return cases_repo.list_cases_for_user(str(user.id))
...
def get_report_url(case_id: str, user: dict = Depends(current_user)):
    case = cases_repo.get_case(case_id)
    _assert_owner(case, user)
    if not case.get("report_storage_path"):
